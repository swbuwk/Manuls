...
class Game {
    constructor() {
        ...
        this.upgrades = [
            ...
            },
            {
                ...
                id: 'grandma',
                click_handler(game) {
                    const button = document.querySelector('#manuls_grandma') //сука все я пофиксил так просто было ток теперь надо со всеми кнопками разобраться бля
                    game.toggle_hide(button, false);
            ...
            },
            {
                ...
                click_handler(game) {
                    ...
                    game.update_counter();
                    document.querySelector("#manulogeddon-audio").play()
                },
            },
        ];
    }
    ...
    update_counter() {
        ...
        const normalized_father_power = (Number(this.father_power)/10).toFixed(2);
        const normalized_grandpa_buff = this.normalize_number(this.grandpa_buff_x); // я тут щас чекай я добавил звук манулогеддона

        if (this.grandpa_hasBuff) {
            this.grandpa_buff_x = (
                this.golden_manuls *
                0.5 * // пук!
                (1 + this.mother_power * 0.01) // пук!
            ).toFixed(2);
        ...
        if (this.golden_manuls <= 1000000000n) {
    ...
    grandma_click() {
        ...
        this.gold_manuls_check(); // в голос купи манулогеддон ток там минус уши ща в голос
        this.update_counter();
    }
    ...
    grandpa_click() {
        ...
        if (this.grandpa_progressbar.value === this.grandpa_progressbar.max) {
            ...
            this.grandpa_buff_x = this.golden_manuls * 0.5 * (1 + this.mother_power * 0.01); // пук! та к и не пон в чем ошибка заключ в 550 в том что нужно было удалить сейв))) сукаа ору когда покупаешь бабушку у нас появляется дедушка, а когда покупаешь деда ничего не происходит
...
window.onload = () => {
    ...
    mygame.load();
    mygame.manuls = 10n**100n;
};

// СЛЕДУЮЩЕЕ ЧТО БУДЕМ ДЕЛАТЬ КАК ПОКУШАЮ, ДОБАВИМ BigInt!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1!!!! да го а че именно хочше
